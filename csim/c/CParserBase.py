...
class CParserBase(Parser):
    # csim-specific change from upstream: reads this instead of sys.argv.
    # Inside csim, sys.argv holds csim's OWN CLI arguments (--path, --lang c,
    # ...), unrelated to this parser's --debug/--no-semantics/--output-*
    # flags -- reading it directly would be silently at the mercy of
    # whatever csim's own argument list happens to contain. Configurable via
    # set_args(), mirroring CLexerBase's same fix.
    _args = []

    def __init__(self, input, output=sys.stdout):
        super().__init__(input, output)
        args = CParserBase._args
