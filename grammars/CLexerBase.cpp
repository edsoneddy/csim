...
CLexerBase::runGccAndMakeStream(antlr4::CharStream *input)
{
    ...
    // Default to --nopp, not real preprocessing -- see CLexerBase.h's class
    // comment for why (csim can't assume gcc/clang is present, and judge
    // submissions have no consistent include paths anyway).
    if (!(vsc || gcc || clang)) {
        nopp = true;
    }
    ...
    // ---- --nopp -----------------------------------------------------------
    // csim-specific change from upstream: no longer writes `inputText` to
    // `outputName` on disk. Upstream did this unconditionally on every call
    // (even in the pass-through case), which meant every single parse wrote
    // a file to the working directory -- wasted I/O per file and a race
    // condition under any concurrent/parallel use, for a debug artifact
    // csim has no use for.
    if (nopp) {
